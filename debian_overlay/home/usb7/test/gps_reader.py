...
hw_manager = Server(f'http://127.0.0.1:8800')

def start_gps():
    print(f'starting gps ...')
    if product in ['td04']:
        results = hw_manager.rpc_ec800m_execute_command('AT+QGPS=1')
        if results['success'] == True or results['response'][0] == '+CME ERROR: 504':
            print(f'starting gps(ec800m) success')
        else:
            print(f'starting gps(ec800m) fail')
            sys.exit(1)
    else:
        print(f'starting gps fail: no gps found for product {product}')
        sys.exit(1)
